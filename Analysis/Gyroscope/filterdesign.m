...
gyropitchfiltdc = sum(gyropitchfilt)/length(gyropitchfilt) * ones(1,length(gyropitchfilt));
gyropitchintdc = sum(gyropitchint)/length(gyropitchint) * ones(1,length(gyropitchint));
gyropitchintfiltdc = sum(gyropitchintfilt)/length(gyropitchintfilt) * ones(1,length(gyropitchintfilt));
% for k = (n+1):L
%     tempsum = 0;
%     %Input Bias Calculation
