...
VAR
  byte newstr[maxstrlength]
obj
  fmath:        "DynamicMathLib.spin"
pub concat(str1, str2) | i,j,k, len1, len2
''DATE:  1-Aug-09
''DPG:   Created.  Need to Test.
  len1 := strsize(str1)
  len2 := strsize(str2)
  i := 0
  j := 0
  k := 0
  wordfill(@newstr, 0, (len1 + len2))
  repeat i from 0 to len1
    byte[newstr][i] := byte[str1][i]
  repeat i from len1 to len2
    byte[newstr][i] := byte[str2][k]
    k++

  if (len1 + len2) => maxstrlength
    return TRUE
  else
    return FALSE
...
''6-APR-09
''DPG:  Inputs is a whole string and what field you want out of it, 0-indexed.  String should start with a "$"
