...
void APM_Compass_Class::Read()
{
  ...
  if (i==6)  // All bytes received?
    ...
    Mag_X = -((((int)buff[2]) << 8) | buff[3]);    // X axis
    Mag_Y = -((((int)buff[0]) << 8) | buff[1]);    // Y axis
    Mag_Z = -((((int)buff[4]) << 8) | buff[5]);    // Z axis

// This is the code from the last release. Not sure why the buffer layout is different from 2.7 and earlier (shown above)
